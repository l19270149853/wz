...
# ---------- 港澳台明确列表（已转为简体） ----------
HMT_SPECIAL = [
    "中旺电视", "东森超视美洲台", "民视新闻台", "台视新闻台",
    "汉天卫视", "KTSF26洛杉矶三藩市", "中旺电视", "东森中国台",
    "八大戏剧", "靖洋戏剧", "龙华经典", "龙华戏剧",
    "东森美洲卫视", "东森美洲HD", "美国城市卫视ICITI",
    "寰宇新闻台", "寰宇新闻台湾台", "镜电视新闻台",
    "东森新闻台", "东森财经新闻台", "三立新闻iNEWS",
    "三立综合台", "民视第一台"
]
...
def normalize_channel(name):
    ...
    clean = re.sub(r'[-\s_高清HD]+', '', clean, flags=re.IGNORECASE)
    return clean.strip()
...
def classify_channel(name, url):
    ...
    if "cctv" in lower or "中央台" in lower:
        return "中央电视"
    # 3. 直辖市卫视 → 归入省级卫视（合并）
    direct_cities = ["北京卫视", "东方卫视", "天津卫视", "重庆卫视", "上海卫视"]
    if any(city in name for city in direct_cities):
        return "省级卫视"          # 改为省级
    # 4. 海峡卫视 -> 省级
    if "海峡卫视" in name:
        return "省级卫视"
    ...
    if "chc" in lower:
        return "省级卫视"
    # 6. 港澳台关键词
    gangaotai = ["凤凰", "tvb", "无线", "港澳", "香港", "澳门", "台湾", "中天", "tvbs", "东森", "华视", "民视", "靖天", "龙华", "美亚", "中视", "八大", "台视", "澳视", "天映", "翡翠", "明珠", "龙祥", "寰宇", "镜电视", "三立", "中旺", "汉天", "ktsf", "美国城市"]
...
def main():
    ...
    # ---- 去重：按 URL 保留速度最快，并统计名称出现次数 ----
    url_info = {}
    for name, url, group, speed in valid:
        std_name = normalize_channel(name)
        if url not in url_info:
            url_info[url] = {'names': set(), 'speed': speed}
        url_info[url]['names'].add(std_name)
        if speed < url_info[url]['speed']:
            url_info[url]['speed'] = speed

    deduped = []
    for url, info in url_info.items():
        name_counter = Counter(info['names'])
        best_name = name_counter.most_common(1)[0][0]
        deduped.append((best_name, url, info['speed']))

    print(f"📌 去重后频道数: {len(deduped)}")

    # 分类
    classified = {}
    for name, url, speed in deduped:
        cat = classify_channel(name, url)
        classified.setdefault(cat, []).append((name, url, speed))
