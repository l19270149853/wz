...
def normalize_channel(name):
    """将各种变体统一为标准名称，并转为简体"""
    name = zhconv.convert(name, 'zh-cn')  # 繁→简
    name = name.strip()
    lower = name.lower()
    # CCTV映射
    cctv_map = {
...
def classify_channel(name, url):
    ...
    if "chc" in lower:
        return "省级卫视"
    # 5. 港澳台关键词（均已改为简体）
    gangaotai = ["凤凰", "tvb", "无线", "港澳", "香港", "澳门", "台湾", "中天", "tvbs", "东森", "华视", "民视", "靖天", "龙华", "美亚", "美亚", "中视","中视","八大","华视","华视","台视",
                 "澳视","澳视","天映","翡翠","明珠","东森","龙祥",
                 "寰宇", "镜电视", "三立", "台视", "中旺", "汉天", "ktsf", "美国城市"]
...
def main():
    ...
    # ---- 新增：繁简转换 + 去重（按URL保留速度最快） ----
    url_best = {}
    for name, url, group, speed in valid:
        std_name = normalize_channel(name)      # 已转简体并标准化
        if url not in url_best or speed < url_best[url][1]:
            url_best[url] = (std_name, speed)
    # 转换为列表
    deduped = [(name, url, speed) for url, (name, speed) in url_best.items()]
